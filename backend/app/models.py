...
class DealershipSftpSettings(Timestamped, Base):
    ...
    tls_certificate_fingerprint: Mapped[str] = mapped_column(String(128), default="")
...
class WebPushSubscription(Timestamped, Base):
...
class PhotoAsset(Timestamped, Base):
    ...
    quality_review_notified_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
