...
def _extend_photoroom_shadow_downward(
    ...
    *,
    ...
    warped_mask = cv2.GaussianBlur(
        warped_mask,
        (0, 0),
        sigmaX=max(1.2, depth * 0.04),
        sigmaY=max(1.8, depth * 0.06),
    )
    ...
    fade[source_height:] = 1 - (
        tail_progress * tail_progress * (3 - 2 * tail_progress)
    )
